...
def intersection_between_two_regions(graph, regions,clauses, split_point):
...
def cheke_factor_node(G,devides,clauses,max_item):
...
def get_regions_z(G,clauses,max_item,R_region,R_subregion):
    ...
    devides = intersection_between_two_regions(G,devides,clauses,max_item)
    devides = [devide for devide in devides if len(devide)>1]
    devides = cheke_factor_node(G,devides,clauses,max_item)
